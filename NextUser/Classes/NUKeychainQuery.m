...
@implementation NUKeychainQuery
...
#ifdef NUKEYCHAIN_ACCESS_GROUP_AVAILABLE
@synthesize accessGroup = _accessGroup;
#endif

#ifdef NUKEYCHAIN_SYNCHRONIZATION_AVAILABLE
...
- (BOOL)save:(NSError *__autoreleasing *)error {
    OSStatus status = NUKeychainErrorBadArguments;
    ...
    if (status == errSecSuccess) {//item already exists, update it!
        ...
        CFTypeRef accessibilityType = [NUKeychain accessibilityType];
...
- (NSMutableDictionary *)query {
    ...
    if ([[self class] isSynchronizationAvailable]) {
        ...
        switch (self.synchronizationMode) {
            case NUKeychainQuerySynchronizationModeNo: {
              value = @NO;
              break;
            }
            case NUKeychainQuerySynchronizationModeYes: {
              value = @YES;
              break;
            }
            case NUKeychainQuerySynchronizationModeAny: {
...
+ (NSError *)errorWithCode:(OSStatus) code {
    ...
    dispatch_once(&onceToken, ^{
        NSURL *url = [[NSBundle bundleForClass:[NUKeychainQuery class]] URLForResource:@"SAMKeychain" withExtension:@"bundle"];
    ...
    switch (code) {
        ...
        case NUKeychainErrorBadArguments: message = @"Some of the arguments were invalid";
            break;
    ...
    return [NSError errorWithDomain:kNUKeychainErrorDomain code:code userInfo:userInfo];
}
